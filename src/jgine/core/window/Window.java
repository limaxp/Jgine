...
public class Window {
	...
	public Window(String title) {
		this(title, Options.RESOLUTION_X.asInt(), Options.RESOLUTION_Y.asInt(), Options.WINDOW_MODE.asInt());
	}
	...
	public Window(String title, int resolutionX, int resolutionY, int mode) {
		...
		Display display = DisplayManager.getDisplay(Options.MONITOR.asInt());
		...
		v_sync(Options.V_SYNC.asBoolean());
		show();
	}
	...
	public void setCursorPos(double x, double y) {
		glfwSetCursorPos(id, x, getSize().y - y);
	}
	...
	public Vector2f getCursorPos() {
		try (MemoryStack stack = MemoryStack.stackPush()) {
			...
			return new Vector2f((float) xPos.get(0), getSize().y - (float) yPos.get(0));
		}
	}
	...
	public Vector2f getResolution() {
		return new Vector2f(resolutionX, resolutionY);
	}
