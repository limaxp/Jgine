...
public class RenderConfiguration implements AutoCloseable {
	...
	public static RenderConfiguration create(float x, float y, float width, float height) {
		...
		int intAliasing = Options.ANTI_ALIASING.asInt();
		if (intAliasing <= 0)
			renderTarget = RenderTarget.createDefaultRenderBuffer(Options.RESOLUTION_X.asInt(),
					Options.RESOLUTION_Y.asInt());
		else
			renderTarget = RenderTarget.createDefaultRenderBufferMultisample(Options.RESOLUTION_X.asInt(),
					Options.RESOLUTION_Y.asInt(), intAliasing);
