...
public class UIList extends UICompound {
	...
	public void load(Map<String, Object> data) {
		this.elementHeight = ObjectUtils.toFloat(data.get("elementHeight"), elementHeight);
		this.reverse = ObjectUtils.toBoolean(data.get("reverse"), reverse);
		this.scrollable = ObjectUtils.toBoolean(data.get("scrollable"), scrollable);
		super.load(data);
	}

	@Override
	public void save(Map<String, Object> data) {
		data.put("elementHeight", elementHeight);
		data.put("reverse", reverse ? "true" : "false");
		data.put("scrollable", scrollable ? "true" : "false");
		super.save(data);
	}
	...
	protected void setChildFunctions(UIObject child) {
		...
		if (child.getScrollFunction() == NULL_VALUE_FUNCTION)
			child.setScrollFunction((_, scroll) -> setScroll(scroll.intValue()));
	}
	...
	public void placeChild(UIObject child, int index) {
		child.setPos(0, 1 - (elementHeight * (index + 1 - scroll)));
		child.setScale(1, elementHeight);
	}

	public void placeChildReverse(UIObject child, int index) {
		child.setPos(0, elementHeight * (index - scroll));
		child.setScale(1, elementHeight);
	}
}
