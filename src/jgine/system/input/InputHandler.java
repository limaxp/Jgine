...
public class InputHandler implements SystemObject {
	...
	private Consumer<Vector2f> mouseMove = (_) -> {
	};
	private Consumer<Float> mouseScroll = (_) -> {
	};
	private Consumer<Vector2f> leftStickMove = (_) -> {
	};
	private Consumer<Vector2f> rightStickMove = (_) -> {
	};
	private Consumer<Float> leftTriggerMove = (_) -> {
	};
	private Consumer<Float> rightTriggerMove = (_) -> {
	};
	...
	protected void checkKeys(InputDevice inputDevice) {
		...
		for (int key : inputDevice.getPressedKeys())
			pressMap.getOrDefault(getId(deviceType, key), (_) -> {
			}).accept(inputDevice.getTimePressed(key));

		for (int key : inputDevice.getReleasedKeys())
			releaseMap.getOrDefault(getId(deviceType, key), (_) -> {
			}).accept(inputDevice.getTimePressed(key));
	}

	@Override
	public void load(Map<String, Object> data) {
		mouse = ObjectUtils.toBoolean(data.get("mouse"), mouse);
		keyboard = ObjectUtils.toBoolean(data.get("keyboard"), keyboard);
		gamepad = ObjectUtils.toInt(data.get("gamepad"), gamepad + 1) - 1;
		active = ObjectUtils.toBoolean(data.get("active"), active);
	}
	...
	public static InputHandler get(String name) {
		return Registry.INPUT.get(name).get();
	}
