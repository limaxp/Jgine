...
import jgine.system.ui.UIScene;
...
public class UIInputHandler extends InputHandler {
	...
	public static final Key KEY_INFO = new Key(Key.KEY_UNKNOWN, Key.KEY_UNKNOWN, Key.MOUSE_BUTTON_RIGHT,
			Key.GAMEPAD_BUTTON_Y);
	...
	protected void init(Entity entity) {
		...
		press(KEY_ENTER, (time) -> onClickKey(time, Key.MOUSE_BUTTON_LEFT)); // TODO pass real key!
		release(KEY_ENTER, (time) -> onReleaseKey(time, Key.MOUSE_BUTTON_LEFT));
		press(KEY_INFO, (time) -> onClickKey(time, Key.MOUSE_BUTTON_RIGHT));
		release(KEY_INFO, (time) -> onReleaseKey(time, Key.MOUSE_BUTTON_RIGHT));
	}
	...
	private void onMouseMove(Vector2f cursorPos) {
		...
		float mouseY = cursorPos.y / windowSize.y;

		focusObject = null;
		for (UIWindow window : getEntity().<UIWindow>getAll(Engine.UI)) {
	...
	private void onClickKey(int time, int key) {
		...
		((UIScene) focusObject.getEntity().scene.getSystem(Engine.UI)).setTopWindow(focusObject);
	}
	...
	private void onClickUp(int time) {
		...
		if (focusObject == null)
			focusObject = getEntity().get(Engine.UI);
	...
	private void focus() {
		...
		focusObject.forParents(focusObjectsNew::add);
