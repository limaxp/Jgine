...
import jgine.utils.memory.NativeResource;
import jgine.utils.memory.Struct;
...
public abstract class ByteBufferSystemScene<S extends EngineSystem<S, O>, O extends Struct & SystemObject>
	...
	public final void remove(int index) {
		...
		if (index != --size) {
			...
			lastEntity.setId(id, size, index);
		}
	}
	...
	protected final void swap(int index1, int index2) {
		...
		relink(index2, first);
		first.setId(id, index1, index2);
		second.setId(id, index2, index1);
	}
