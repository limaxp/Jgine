...
public class TimeBuffer {
	...
	protected Runnable[] objectArray;
	protected int size;

	public TimeBuffer(int capacity) {
		timeArray = new long[capacity];
		objectArray = new Runnable[capacity];
		size = 0;
	}

	public int add(long time, Runnable element) {
	...
	protected void add(long time, Runnable element, int index) {
	...
	public int remove(Runnable element) {
	...
	public int getIndex(Runnable element) {
		for (int i = 0; i < size; i++) {
	...
	public void update(long time, Consumer<Runnable> func) {
		if (size == 0)
			return;
		if (time < timeArray[0])
			return;
